...
type ChnageProfileModelProps = {
  chnageImageModel: boolean;
  setChnageImageModel: React.Dispatch<React.SetStateAction<boolean>>;
};

const ChnageProfileModel: React.FC<ChnageProfileModelProps> = ({
  setChnageImageModel,
  chnageImageModel,
}) => {
  ...
  const onChangeImage = async () => {
    const userProfileImageRef = doc(firestore, "users", `${user!.uid}`);

    const imageRef = ref(storage, `profiles/${user!.uid}/profileimg`);
    await uploadString(imageRef, selectedFile, "data_url");
    const downloadURL = await getDownloadURL(imageRef);

    updateDoc(userProfileImageRef, {
...
export default ChnageProfileModel;
